...
audit，發現兩個複合的根因：
（1）v10/v12/v13 用的「258M」根本是**錯的資料集**——HPC 上的舊版（3,507 物種），
   而非正確的 CrucialX9 1,535 物種版（兩者讀數量都是 258M，路徑被混淆）；
（2）train_ddp.py 的 `_read_seq` 只讀第一行，偏偏 258M FASTA 是 60 欄 wrap，
   導致每條 150bp read 被截成 60bp；
（3）v9 66.6% 這個 baseline 本身有 test/train 洩漏，被高估。
修正方案：改用正確的 CrucialX9 1,535 物種資料（已於 6/17 上傳 Nano4）+ 修 reader +
重建乾淨 test set，這樣才能真正測 volume/balance 這個問題。
...
data 的 audit，找到兩個複合的問題：

第一，**資料集用錯了**。v10/v12/v13 餵進去的 258M，其實是 HPC 上的舊版（3,507 物種），
不是我以為的 CrucialX9 正確版（1,535 物種）。兩個檔案讀數量都接近 258M，路徑被我混淆了。
正確的 CrucialX9 1,535 物種資料是 6/17 才傳到 Nano4 的，v10 是 6/11 就丟出去的，
那時候根本還沒有。

第二，**reader bug**。258M 的訓練走 train_ddp.py 的 lazy loader，那個 `_read_seq`
只讀每筆 record 的第一行；而這個 258M FASTA 是 60 欄 wrap 的，所以每條 150bp 的 read
只被讀進前 60bp——模型實際只看到 40% 的序列。v9 走的是 train.py 的 `_parse_fasta`
（會把多行接起來），加上 50M 的檔本身是單行的，所以讀到完整 150bp。

另外我也發現 66.6% 這個 baseline 有問題：100K test 的 read 100% 落在 50M 訓練集裡，
是洩漏高估的。今天的工作就是切換正確資料、把 reader 修好、重建乾淨 test set，再重測。
...
右邊是真正的診斷，我在 6/18 確認的，找到兩個複合的根因。

**根因一（最關鍵）：資料集用錯了。** v10/v12/v13 餵的是 HPC 上的舊版 258M（3,507 物種），
不是正確的 CrucialX9 1,535 物種版。兩個檔案的讀數量都接近 258M，路徑被混淆了。
正確的 CrucialX9 資料是 6/17 才傳到 Nano4，v10 是 6/11 就丟出去——那時候根本還沒到。
訓練用 3,507 物種的 genus label 分佈，但測試是 1,535 物種的空間，當然是對不上的。

**根因二：讀檔 bug（複合傷害）。** 258M 的訓練走的是 train_ddp.py 的 lazy loader，
裡面的 `_read_seq` 只讀每筆 record 的第一行；而這個 258M FASTA 是 60 欄 wrap 的
（150bp 拆成 60+60+30 三行），所以模型每條 read 只讀到前 60bp，等於丟掉 60% 的序列。
v9 走的是 train.py 的 `_parse_fasta`（會把多行接起來），而且 50M 的檔本身是單行的，
所以讀到完整 150bp。

**根因三：baseline 本身有洩漏。** 100K 測試集的 seq_id 我抽查 300 筆，
**300 筆全部都在 50M 訓練資料裡**——66.6% 是被高估的。

「genus 不平衡」排除：v9 是 349:1、v13 是 400:1，幾乎一樣，不平衡解釋不了 22pp 的差距。
class_weights 只是讓 v10 在已經很爛的基礎上額外更糟（40% vs 45%）。
...
- **v10**：258M（**HPC 舊版，3,507 物種**）、warm-start、**加權**、epoch 12 early stop → 40.1%，collapse。
- **v12**：258M（同上，HPC 舊版）、warm-start、**不加權**、只跑到 3 epochs → 45.4%。
- **v13**：258M（同上，HPC 舊版）、從零、**不加權**、完整 25 epochs → 44.5%。

三個備註：
所有 258M 實驗都用了 HPC 舊版（3,507 物種）而非正確的 CrucialX9 1,535 物種版——這是最主要的根因。
...
## Slide 6 — Key Finding: Two Compounding Bugs

這張是這週最重要的 takeaway，而且結論跟我原本以為的不一樣。

這 22pp 的差距不是來自資料不平衡，而是兩個同時存在的 bug：

**Bug 1——資料集根本用錯了（最主要）**。v10/v12/v13 用的 258M 是 HPC 上的舊版
（3,507 物種），不是正確的 CrucialX9 1,535 物種版。兩份資料的物種組成、genus 分佈
根本不同，訓練出來的 classifier 對 1,535 物種的 test set 當然對不上。

**Bug 2——讀檔截斷（複合傷害）**。即使拿到正確的資料，train_ddp.py 的 `_read_seq`
只讀 record 的第一行；而這個 258M FASTA 是 60 欄 wrap 的，所以每條 read 只被讀入
前 60bp（150bp 的 40%）。v9 用的 train.py `_parse_fasta` 會把多行接起來，
加上 50M 的檔本身是單行的，所以讀到完整 150bp。

右邊三個結論：

第一，**volume 跟 balance 都還沒被真正測過**。所有 258M 的 run 用的是錯的資料集，
在修掉這兩個 bug 之前，我們對「資料量」或「平衡」下不了任何結論。

第二，**66.6% 的 baseline 是漏的**。100K test 完全落在 50M 訓練集裡（100% 重疊），
所以 v9 這個數字被高估，真正乾淨的 baseline 會更低。

第三，**正確的資料已就位**。CrucialX9 1,535 物種版（258M，47GB）6/17 已上傳 Nano4，
修好 reader 之後就可以直接跑正確的 258M 實驗。

一句話總結：這週最大的收穫不是一個分數，而是找到兩個讓所有 258M 實驗同時失真的 bug，
而且正確的材料已經備好了。
...
**第一步——修掉兩個 bug + 重建 baseline**：
- 切換資料：改用正確的 CrucialX9 1,535 物種版（已在 Nano4：
  `/work/ymj1123ntu/data/labeled_multi_level_1535sp/reads.fa`，47GB，6/17 上傳完成）。
- 修 `_read_seq`：讓它讀到下一個 `>` 為止，把 wrap 的多行接起來（因為 CrucialX9 也是
  60 欄 wrap，reader bug 同樣會發生），並在訓練啟動時確認樣本長度是 150bp。
- 重建一個**乾淨的 100K test set**：seq_id 與訓練資料完全不重疊，然後用它把 v9
  重新評估一次，得到一個誠實的 baseline（會比 66.6% 低）。

**第二步——用正確資料 + 完整 150bp 重跑**：
- 正確的 258M full-length（class_weights=false）：這才是第一次真正在測「資料量」。
...
主要有三個原因同時發生。第一，v10/v12/v13 用的 258M 根本是錯的資料集——HPC 上的舊版
（3,507 物種），而不是正確的 CrucialX9 1,535 物種版。兩者雖然讀數量接近，但物種組成和
genus 分佈都不同，訓練出來的模型對 1,535 物種的 test set 當然無法對應。第二，train_ddp.py
的 `_read_seq` 只讀第一行，而這個 258M FASTA 是 60 欄 wrap 的，所以每條 read 只讀到
60bp，丟掉 60% 的序列。v9 走的是 train.py 的 `_parse_fasta`（多行接起來），加上 50M
的檔本身是單行的，所以讀到完整 150bp。第三，66.6% 本身也有 test/train 洩漏，是高估的。
