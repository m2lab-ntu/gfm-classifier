...
#SBATCH --mem=64G
#SBATCH -t 02:00:00
#SBATCH -o /work/ymj1123ntu/logs/rctta_3way-%j.out
#SBATCH -e /work/ymj1123ntu/logs/rctta_3way-%j.err
set -uo pipefail
R=/work/ymj1123ntu/gfm-classifier
T=/work/ymj1123ntu/taiwania2_testset
TF=$T/reads_clean_common.fa; TL=$T/labels_clean_common.tsv
...
cd $R/scripts
run(){ echo "########## $1 (RC-TTA) ##########"; python -u run_genus_rctta.py --config "$2" --checkpoint "$3" --test_fasta "$TF" --test_labels "$TL" --train_labels "$4" --out_dir "$T/rctta_$1" 2>&1; }
