...
#SBATCH -J nt_ov6mer
#SBATCH -p gp4d
#SBATCH --nodes=1 --ntasks-per-node=8 --gres=gpu:8 --cpus-per-task=4 --mem=0
#SBATCH -t 4-00:00:00
...
mkdir -p ${CKPT_DIR}
module load miniconda3/conda24.5.0_py3.9; source $(conda info --base)/etc/profile.d/conda.sh; conda activate gfm
export HF_HOME=/work/ymj1123ntu/.cache/huggingface HUGGINGFACE_HUB_CACHE=/work/ymj1123ntu/.cache/huggingface
export OMP_NUM_THREADS=4 PYTHONUNBUFFERED=1 NCCL_DEBUG=WARN
...
srun python train_ddp.py --config ${CONFIG} ${RES} --time_limit_sec 338400 2>&1
