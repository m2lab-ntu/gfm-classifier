...
#SBATCH --job-name=mt50m_soil_6mer
...
LAST=$(ls -td ${EXP_BASE}/genus_50M_soil_6mer_*/checkpoints/*_ckpt_bt_*.pt 2>/dev/null | head -1)
RESUME_DIR=""
...
if [ -n "${RESUME_DIR}" ]; then
    ...
    python3 -u train.py \
        experiment_name=genus_50M_soil_6mer \
        experiment_base_dir=${EXP_BASE} \
        cfg_path=/work/ymj1123ntu/mt50m_soil/config/config_genus_50M_soil_6mer.yaml \
